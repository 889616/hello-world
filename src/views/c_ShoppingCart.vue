<template>
	<div>
		<header>
			<div class="headerTitlle" style="line-height:24px;font-size:16px;">
				<p @click="tz()">
		...
		<section v-if="!loading">
			<div class="contentWrap" v-for="(item,index) in shoppingList" @click="changeChooiseStatus(item.id,index)" :key='index'>
				...
				<div class="shoppingInfo">
					...
					<div class="goodsInfo">
						...
						<p>
							￥{{item.price}}
							<span style="text-align: right;color: #666;">x{{item.count}}</span>
						</p>
					</div>
				</div>
			</div>
		...
		<div v-if="shoppingList.length">
			<footer v-if="!manageStatus">
				...
				<span @click="tzy"><a style="color:#00f">修改地址</a></span>
				<div class="price">
					共：
					<span>￥{{price}}</span>
	...
	export default {
		...
		methods: {
			...
			deleteGoods() {
				...
				Dialog.confirm({
						title: "删除",
						message: "确定删除" + goodsIdList.length + "个商品吗？"
					})
					.then(() => {
						// on confirm
			...
			tzy() {
				this.$router.push("/c_dizhibianji")
			}
		}
	};
</script>
<style scoped>
	...
	.spinner>div {
		...
		border-radius: 100%;
		display: inline-block;
		-webkit-animation: bouncedelay 5.4s infinite ease-in-out;
		animation: bouncedelay 5.4s infinite ease-in-out;
